...
def popup():
   ...
   pop.iconphoto(False, PhotoImage(file="./icon.png"))
   ...
   openfilebtn.place(relx=0.34, rely=0.5, anchor=CENTER)
...
def open_file():
   file_to_open = f'{savedir}/{filename}{open_format}'
   opener = "open" if sys.platform == "darwin" else "xdg-open"
   subprocess.call([opener, file_to_open])

def open_settings():
    opener = "open" if sys.platform == "darwin" else "xdg-open"
    subprocess.call([opener, '.env'])
...
class MainGUI(Tk):
    def __init__(self):
        ...
        e  = tkinter.Entry(self.root, width=75)
        ...
        self.log_widget = ScrolledText(self.root, height=2, width=75, font=("consolas", "10", "normal"))
